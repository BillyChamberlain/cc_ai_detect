...
const state = {
  ...
  /*
    Controls the initial face-size lock in Stage 2.
  */
  faceLockStartedAt: null,
  wasFaceActive: false,

  /*
    Persistent Stage 3 transition:
    - null: Stage 3 has never started
    - number: first valid phone detection timestamp
  */
  phoneTriggerTime: null,

  /*
    Persistent stage, only moves forward:
    1 -> 2 -> 3 -> 4
  */
  stage: 1,
...
/*
  This function advances stages but never moves backward.
*/
function updatePersistentStage(now) {
  ...
  /*
    Stage 2 begins after the Stage 1 draw period.
  */
  if (
    state.stage === 1 &&
    elapsed >= CONFIG.CAMERA_STAGE_START_MS
  ) {
    state.stage = 2;
  }

  /*
    Stage 3 begins on first valid phone detection.
    phoneTriggerTime is assigned inside detect().
  */
  if (
    state.stage < 3 &&
  ...
  /*
    Stage 4 begins after the Stage 3 fog-delay countdown.
  */
  if (
    state.stage === 3 &&
    state.phoneTriggerTime !== null &&
...
function mapModelPointToScreen(modelX, modelY) {
  ...
  const normalizedX = modelX / CONFIG.MODEL_INPUT_SIZE;
  const normalizedY = modelY / CONFIG.MODEL_INPUT_SIZE;

  /*
    Inference itself is mirrored exactly once in
    drawVideoToInferenceCanvas(), matching CSS scaleX(-1).
  */
  return {
    x: videoRect.x + normalizedX * videoRect.width,
    y: videoRect.y + normalizedY * videoRect.height,
  };
}
...
async function loadFaceModel() {
  if (state.faceSession || state.faceModelLoading) return;
  ...
  try {
    ...
    state.faceInputName = state.faceSession.inputNames[0];
  } finally {
...
async function loadPhoneModel() {
  if (state.phoneSession || state.phoneModelLoading) return;
  ...
  try {
    ...
    state.phoneInputName = state.phoneSession.inputNames[0];
  } finally {
...
async function startCamera() {
  if (state.cameraStarted || state.cameraStarting) return;
...
function createInputTensor() {
  ...
  for (let pixelIndex = 0; pixelIndex < channelSize; pixelIndex++) {
...
function calculateIoU(first, second) {
  ...
  const overlapLeft = Math.max(firstLeft, secondLeft);
  const overlapTop = Math.max(firstTop, secondTop);
  const overlapRight = Math.min(firstRight, secondRight);
  const overlapBottom = Math.min(firstBottom, secondBottom);
  ...
  const firstArea =
    (firstRight - firstLeft) * (firstBottom - firstTop);

  const secondArea =
    (secondRight - secondLeft) *
    (secondBottom - secondTop);

  return overlapArea / (firstArea + secondArea - overlapArea + 1e-6);
}
...
function nonMaximumSuppression(detections) {
  ...
  while (candidates.length > 0) {
    ...
    for (let index = candidates.length - 1; index >= 0; index--) {
      if (
        calculateIoU(best, candidates[index]) >
...
function parseYoloOutput(
  ...
  /*
    ...
    [1, detectionCount, 6]
    Detection: [x1, y1, x2, y2, confidence, classId]
  */
  if (dims.length === 3 && dims[2] === 6) {
    ...
    return nonMaximumSuppression(detections)[0] || null;
  }

  /*
    Raw YOLO output:
    [1, 4 + classCount, anchorCount]
    or [1, anchorCount, 4 + classCount]
  */
  if (dims.length !== 3) {
    console.warn("Unexpected YOLO output dimensions:", dims);
    return null;
  }

  const attributesFirst = dims[1] < dims[2];
  const attributes = attributesFirst ? dims[1] : dims[2];
  const anchors = attributesFirst ? dims[2] : dims[1];

  if (attributes < 5) {
    console.warn("Invalid YOLO output shape:", dims);
    return null;
  }
  ...
  for (let anchor = 0; anchor < anchors; anchor++) {
    const read = (attribute) => {
      if (attributesFirst) {
        return data[attribute * anchors + anchor];
      }

      return data[anchor * attributes + attribute];
    };
    ...
    for (let attribute = 4; attribute < attributes; attribute++) {
...
async function detect(now) {
  if (!state.started || !state.cameraStarted) return;
  if (!state.faceSession || !video.videoWidth) return;
  ...
  try {
    ...
    if (state.phoneSession) {
      ...
      if (phoneDetection) {
        ...
        /*
          First confirmed phone detection permanently begins Stage 3.
        */
...
function updateStageUi(now) {
  ...
  if (stage === 1) {
    phaseLabel.textContent = "Stage 01 · Drawing blur";
    modelLabel.textContent = "Camera offline";
    return;
  }

  if (stage === 2) {
    phaseLabel.textContent = "Stage 02 · Circular orbit: The threat of lurks, try bringing a phone into frame";
    ...
    if (state.cameraStarting) {
      ...
      modelLabel.textContent = "Face presence detected";
    } else {
      modelLabel.textContent = "Searching for face";
    }
  ...
  if (stage === 3) {
    ...
    phaseLabel.textContent = "Stage 03 · Phone response, blur covers face";
  ...
  phaseLabel.textContent = "Stage 04 · Permanent fog everytime phone is brought";
...
function updateBlurBox(now) {
  ...
  let targetWidth =
    state.width * CONFIG.INITIAL_BOX_WIDTH_RATIO;

  let targetHeight =
    state.height * CONFIG.INITIAL_BOX_HEIGHT_RATIO;

  if (stage === 1) {
    targetX =
      state.width / 2 +
      Math.sin(now * 0.00032) * state.width * 0.017;

    targetY =
      state.height / 2 +
      Math.cos(now * 0.00028) * state.height * 0.014;
  }

  const faceActive = isFaceActive(now);

  if (faceActive && !state.wasFaceActive) {
  ...
  if (stage === 2) {
    if (faceActive) {
      ...
      const isInFaceLock =
        faceLockAge < CONFIG.FACE_LOCK_DURATION_MS;
      ...
      if (isInFaceLock) {
        ...
        const orbitRadius = Math.max(
          Math.max(face.width, face.height) * 1.45,
          Math.min(state.width, state.height) *
            CONFIG.ORBIT_RADIUS_RATIO
        );

        const orbitAngle = elapsed * CONFIG.ORBIT_SPEED;
  ...
  /*
    Both Stage 3 and Stage 4 persist after their entry.
    The blur behavior is still controlled by current phone presence:
    - Phone present: blur covers face.
    - Phone absent: blur returns to circular orbit.
  */
  if (stage === 3 || stage === 4) {
    if (faceActive) {
      const face = getFaceScreenData();
      ...
      if (isPhoneActive(now)) {
        ...
        targetWidth = face.width * CONFIG.FRONT_BOX_SCALE;
        targetHeight = face.height * CONFIG.FRONT_BOX_SCALE;
      } else {
        const orbitRadius = Math.max(
          Math.max(face.width, face.height) * 1.45,
          Math.min(state.width, state.height) *
...
function createBoxPath(ctx, x, y, width, height, radius) {
...
function drawInitialConstruction(ctx, now) {
  const elapsed = getElapsedMs();

  const progress = easeOutCubic(
    elapsed / CONFIG.INITIAL_DRAW_DURATION_MS
  );
  ...
  ctx.save();
  ctx.filter = `blur(${CONFIG.PRIVACY_BLUR_AMOUNT}px)`;

  for (let line = 0; line < visibleLines; line++) {
    const lineProgress = line / totalLines;
    const lineY = top + lineProgress * height;

    const unstableStart =
      left +
      Math.sin(line * 1.72 + now * 0.006) *
        (1 - progress) *
        width *
        0.12;
    ...
    const lineWidth =
      width *
      (0.72 +
        Math.sin(line * 1.27 + now * 0.003) * 0.12);

    ctx.globalAlpha =
      0.18 +
      Math.sin(line * 0.9 + now * 0.004) * 0.04;
    ...
    ctx.fillRect(
      ...
      lineY,
      lineWidth,
      Math.max(3, (height / totalLines) * 1.65)
    );
  }
  ...
  const drawnPerimeter = perimeter * progress;

  ctx.lineWidth = 1.2;
  ctx.strokeStyle = "rgba(214, 226, 255, 0.56)";
  ctx.shadowColor = "rgba(119, 154, 255, 0.95)";
  ctx.shadowBlur = 15;

  ctx.beginPath();

  let remaining = drawnPerimeter;

  if (remaining > 0) {
    const topLength = Math.min(width, remaining);
    ctx.moveTo(left, top);
    ctx.lineTo(left + topLength, top);
    remaining -= topLength;
  }

  if (remaining > 0) {
    const rightLength = Math.min(height, remaining);
    ctx.moveTo(left + width, top);
    ctx.lineTo(left + width, top + rightLength);
    remaining -= rightLength;
  }

  if (remaining > 0) {
    const bottomLength = Math.min(width, remaining);
    ctx.moveTo(left + width, top + height);
    ctx.lineTo(left + width - bottomLength, top + height);
    remaining -= bottomLength;
  }

  if (remaining > 0) {
    const leftLength = Math.min(height, remaining);
    ctx.moveTo(left, top + height);
    ctx.lineTo(left, top + height - leftLength);
  }
  ...
  if (progress > 0.72) {
    const settledOpacity = (progress - 0.72) / 0.28;
...
function drawAbstractBlurField(
  ...
  createBoxPath(ctx, x, y, width, height, 8);
  ctx.clip();

  ctx.filter = `grayscale(1) contrast(1.5) blur(${CONFIG.PRIVACY_BLUR_AMOUNT}px)`;
  ...
  for (let block = 0; block < blocks; block++) {
    const blockX =
      ...
      ((block * 71) % width) -
      width * 0.16 +
      Math.sin(now * 0.001 + block) * 20;

    const blockY =
      top +
      ((block * 41) % height) -
      height * 0.14 +
      Math.cos(now * 0.0013 + block) * 16;

    const blockWidth = width * randomRange(0.13, 0.34);
    const blockHeight = height * randomRange(0.2, 0.58);
  ...
  drawBoxBoundary(ctx, x, y, width, height, opacity);
}
...
function drawCameraPrivacyBlur(
  ...
  if (!state.cameraStarted || !video.videoWidth) {
    drawAbstractBlurField(ctx, x, y, width, height, 1, now);
    return;
  }

  const softness = CONFIG.EDGE_SOFTNESS;
  const paddedWidth = Math.ceil(width + softness * 2);
  const paddedHeight = Math.ceil(height + softness * 2);
  ...
  const paddedScreenLeft = screenLeft - softness;
  const paddedScreenTop = screenTop - softness;

  const videoRect = getVideoCoverRect();

  let normalizedLeft =
    (paddedScreenLeft - videoRect.x) / videoRect.width;

  const normalizedTop =
    (paddedScreenTop - videoRect.y) / videoRect.height;
  ...
  /*
    Convert from displayed mirrored video coordinates
    to raw video source coordinates.
  */
  if (CONFIG.MIRROR_CAMERA) {
    normalizedLeft =
      1 - normalizedLeft - normalizedWidth;
  }

  const sourceX = normalizedLeft * video.videoWidth;
  const sourceY = normalizedTop * video.videoHeight;
  ...
  blurCtx.clearRect(0, 0, paddedWidth, paddedHeight);

  /*
    Source coordinates are already mirror-corrected,
    so do not flip blurCtx a second time.
  */
  blurCtx.drawImage(
    video,
    sourceX,
  ...
  createBoxPath(ctx, x, y, width, height, 7);
  ctx.clip();
  ...
  for (let pass = 0; pass < CONFIG.PRIVACY_BLUR_PASSES; pass++) {
    ctx.globalAlpha = pass === 0 ? 1 : 0.52;
  ...
  drawBoxBoundary(ctx, x, y, width, height, 1);
}

function drawBoxBoundary(ctx, x, y, width, height, opacity) {
  ctx.save();

  createBoxPath(ctx, x, y, width, height, 7);

  ctx.lineWidth = 1;
  ctx.strokeStyle = `rgba(240, 240, 240, ${0.24 * opacity})`;
  ctx.shadowColor = `rgba(90, 90, 90, ${0.55 * opacity})`;
  ctx.shadowBlur = 12;
...
function drawBlurBox(ctx, now) {
  ...
  if (stage === 1) {
    drawInitialConstruction(ctx, now);
    return;
  }
...
function addFogPuff(now) {
  ...
  const originX = face ? face.x : state.boxX;
  const originY = face ? face.y : state.boxY;

  const spread = face
    ? Math.max(
        Math.max(face.width, face.height) * 1.7,
        190
      )
    : Math.min(state.width, state.height) * 0.24;

  state.fogPuffs.push({
    x: originX + randomRange(-spread, spread),
    y: originY + randomRange(-spread * 0.56, spread * 0.56),
    radius: randomRange(spread * 0.33, spread * 0.92),
    alpha: randomRange(0.018, 0.052),
    hue: randomRange(202, 239),
    rotation: randomRange(0, Math.PI * 2),
    createdAt: now,
  });

  if (state.fogPuffs.length > CONFIG.MAX_FOG_PUFFS) {
...
function updateFog(now) {
  /*
    Stage 4 is persistent. It never turns back into another stage.
    New fog appears only while a phone is currently visible.
  */
  if (getCurrentStage() !== 4 || !isPhoneActive(now)) {
    return;
  }
...
function drawFog(ctx, now) {
  ...
  for (const puff of state.fogPuffs) {
    const ageSeconds = (now - puff.createdAt) / 1000;

    const x =
      puff.x +
      Math.sin(ageSeconds * 0.37 + puff.rotation) * 11;

    const y =
      puff.y +
      Math.cos(ageSeconds * 0.31 + puff.rotation) * 7;
...
function drawAmbientField(ctx, now) {
  const pulse = 0.5 + Math.sin(now * 0.00081) * 0.5;
  const radius = Math.max(state.width, state.height) * 0.8;
  ...
  field.addColorStop(0.55, "rgba(9, 10, 34, 0.025)");
...
function animationLoop(now) {
  ...
  visualCtx.clearRect(0, 0, state.width, state.height);

  /*
    Move Stage 1 -> Stage 2 after 15 seconds.
  */
  updatePersistentStage(now);

  /*
    Start camera/model loading once Stage 2 is entered.
  */
  if (state.started && getCurrentStage() >= 2) {
    beginStageTwo();
  }

  /*
    A phone detection can assign phoneTriggerTime inside detect().
    Calling stage update before UI makes the following frame
    immediately show the correct Stage 3 label.
  */
  updatePersistentStage(now);
  ...
  drawAmbientField(visualCtx, now);
  drawBlurBox(visualCtx, now);
  drawFog(visualCtx, now);
